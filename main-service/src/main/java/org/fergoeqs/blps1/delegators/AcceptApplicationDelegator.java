...
@Component("AcceptApplicationDelegator")
@RequiredArgsConstructor
public class AcceptApplicationDelegator implements JavaDelegate {
    ...
    public void execute(DelegateExecution execution) {
        transactionTemplate.execute(status -> {
            ...
            ApplicationResponse response = applicationService.acceptApplication(applicationId, -1L);
            saveResultsToExecution(execution, response);
            logAfterAccept(response);
    ...
    private void saveResultsToExecution(DelegateExecution execution, ApplicationResponse response) {
        execution.setVariable("acceptedApplicationId", response.id());
    ...
    private void logAfterAccept(ApplicationResponse response) {
        System.out.println("=== Заявка успешно принята ===");
